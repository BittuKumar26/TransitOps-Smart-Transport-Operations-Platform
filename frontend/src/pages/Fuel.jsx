...
import { createFuelLog, deleteFuelLog, getFuelLogs } from '../api/fuelApi.js';
...
export default function Fuel() {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      setError('');
      const [fuelResponse, vehicleResponse] = await Promise.all([getFuelLogs(), getVehicles()]);
      setFuelLogs(fuelResponse.data.data ?? fuelResponse.data);
      setVehicles(vehicleResponse.data.data ?? vehicleResponse.data);
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Failed to load fuel logs');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setSubmitting(true);
      setError('');
      await createFuelLog({ ...form, liters: Number(form.liters), cost: Number(form.cost) });
      setForm({ vehicleId: '', liters: '', cost: '' });
      await loadData();
    } catch (requestError) {
      setError(requestError?.response?.data?.message || 'Failed to create fuel log');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setError(requestError?.response?.data?.message || 'Failed to delete fuel log');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl bg-ink px-6 py-8 text-white shadow-xl">
        <p className="text-sm uppercase tracking-[0.35em] text-teal-300">Cost tracking</p>
        <h2 className="mt-2 text-3xl font-semibold">Fuel Logs</h2>
      </section>
      ...
      <section className="grid gap-6 xl:grid-cols-[420px_1fr]">
        <Card>
          <h3 className="text-lg font-semibold text-ink">Add fuel entry</h3>
          <form className="mt-4 space-y-4" onSubmit={handleSubmit}>
            ...
            <Input label="Liters" type="number" min="0" value={form.liters} onChange={(event) => setForm((current) => ({ ...current, liters: event.target.value }))} />
            ...
            <Button type="submit" className="w-full bg-teal-600" disabled={submitting}>
              {submitting ? 'Saving...' : 'Create Fuel Log'}
            </Button>
          </form>
        </Card>
        ...
        <Card className="overflow-hidden p-0">
          <div className="border-b border-slate-200 px-5 py-4">
            <h3 className="text-lg font-semibold text-ink">Fuel records</h3>
          </div>
          ...
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {fuelLogs.length === 0 ? (
                    <tr><td className="px-4 py-8 text-slate-500" colSpan={5}>No fuel logs found.</td></tr>
                  ) : (
                    fuelLogs.map((row) => (
                      <tr key={row._id} className="border-t border-slate-100">
                        <td className="px-4 py-3 text-slate-700">{row.vehicleId?.vehicleName || '-'}</td>
                        <td className="px-4 py-3 text-slate-700">{row.liters}</td>
